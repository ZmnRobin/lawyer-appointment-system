...
function VideoCall() {
  ...
  const userVideo = useRef(null);  // Initialize with null instead of undefined
  const connectionRef = useRef(null);  // Initialize with null instead of undefined
  const myVideo = useRef(null); 

  useEffect(() => {
    navigator.mediaDevices.getUserMedia({ video: true, audio: true }).then((stream) => {
      setStream(stream);
      if (myVideo.current) {  // Check if myVideo.current is not undefined
